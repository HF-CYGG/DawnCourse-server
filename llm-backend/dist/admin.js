...
import { disableScriptRelease, publishScriptRelease, rollbackScriptRelease } from "./scriptReleaseAdminService.js";
import { createUploadedScriptRelease, revalidateScriptRelease } from "./scriptUpload.js";
...
export async function registerAdminRoutes(app, deps = {}) {
    ...
    app.get("/api/v1/admin/session", { preHandler: authRequiredFn }, async (request) => {
    ...
    app.post("/api/v1/admin/users", { preHandler: authRequiredFn }, async (request, reply) => {
        const body = (request.body || {});
        const username = String(body.username || "").trim();
        const password = String(body.password || "");
    ...
    app.post("/api/v1/admin/users/rename", { preHandler: authRequiredFn }, async (request, reply) => {
        const body = (request.body || {});
    ...
    app.post("/api/v1/admin/users/password", { preHandler: authRequiredFn }, async (request, reply) => {
        const body = (request.body || {});
        const username = String(body.username || "").trim();
    ...
    app.post("/api/v1/admin/users/delete", { preHandler: authRequiredFn }, async (request, reply) => {
        const body = (request.body || {});
        const username = String(body.username || "").trim();
    ...
    app.post("/api/v1/admin/events/token", { preHandler: authRequiredFn }, async (request) => {
        const token = id("evt");
    ...
    app.post("/api/v1/admin/client_error", { preHandler: authRequiredFn }, async (request) => {
        const body = (request.body || {});
    ...
    app.post("/api/v1/admin/repair/issues/:id/run-test", { preHandler: authRequiredFn }, async (request) => {
        const issueId = request.params.id;
        const result = await runReplayOnlyFn(issueId, request.adminUser || "admin");
        return apiOk({ ...result, testedBy: request.adminUser || "admin" });
    });
    app.post("/api/v1/admin/repair/issues/:id/retry", { preHandler: authRequiredFn }, async (request, reply) => {
        const issueId = request.params.id;
    ...
    app.post("/api/v1/admin/repair/issues/:id/force-repair", { preHandler: authRequiredFn }, async (request, reply) => {
        const issueId = request.params.id;
    ...
    app.post("/api/v1/admin/repair/issues/:id/run", { preHandler: authRequiredFn }, async (request, reply) => {
        const issueId = request.params.id;
        const result = await startRepairJobFn(issueId, { actor: request.adminUser || "admin", bypassMinQueue: true });
        if (!result.started && result.reason) {
            const code = result.reason === "问题不存在" ? 404 : 409;
    ...
    app.get("/api/v1/admin/scripts", { preHandler: authRequiredFn }, async () => {
        const rows = await query(`SELECT r.*, a.content_sha256, a.signature, parent.version AS parent_version,
    ...
    app.post("/api/v1/admin/scripts/uploads", { preHandler: authRequiredFn }, async (request, reply) => {
        try {
            const result = await createUploadedScriptReleaseFn((request.body || {}), request.adminUser || "admin");
    ...
    app.post("/api/v1/admin/scripts/releases/:id/revalidate", { preHandler: authRequiredFn }, async (request, reply) => {
        const releaseId = request.params.id;
        try {
            const result = await revalidateScriptReleaseFn(releaseId, (request.body || {}), request.adminUser || "admin");
            return apiOk(result);
        }
        catch (error) {
            return sendScriptReleaseError(reply, error);
    ...
    app.get("/api/v1/admin/scripts/releases/:id", { preHandler: authRequiredFn }, async (request, reply) => {
        const releaseId = request.params.id;
        const releaseResult = await query(`SELECT r.*, a.content, a.content_sha256, a.signature, a.alg,
       ...
       WHERE r.release_id = $1 LIMIT 1`, [releaseId]);
        ...
        if (!release)
            return reply.code(404).send(apiError(404, "脚本版本不存在"));
        const [dependencies, validations, audits, metrics] = await Promise.all([
            query(`SELECT d.load_order,dr.release_id,dr.name,dr.version,da.content_sha256
         ...
         WHERE d.release_id = $1 ORDER BY d.load_order`, [releaseId]),
            ...
            query(`SELECT metric_date,school_id,school_system_type,event_type,error_code,SUM(event_count)::bigint AS event_count,MAX(last_event_at) AS last_event_at
         FROM script_activation_metrics WHERE release_id = $1
         GROUP BY metric_date,school_id,school_system_type,event_type,error_code
         ORDER BY metric_date DESC,last_event_at DESC`, [releaseId])
        ]);
        return apiOk({
    ...
    app.get("/api/v1/admin/scripts/activation-metrics", { preHandler: authRequiredFn }, async (request) => {
        ...
        const rows = await query(`SELECT m.metric_date,m.release_id,r.script_key,r.name,m.school_id,m.school_system_type,
       ...
       ORDER BY m.metric_date DESC,last_event_at DESC LIMIT 1000`, [releaseId]);
        return apiOk({ list: rows.rows });
    });
    app.get("/api/v1/admin/unmatched-schools", { preHandler: authRequiredFn }, async () => {
        const rows = await query(`SELECT sp.school_id,sp.school_name,sp.school_system_type,sp.status,sp.updated_at,
    ...
    app.post("/api/v1/admin/scripts/releases", { preHandler: authRequiredFn }, async (request, reply) => {
        const body = (request.body || {});
        ...
        if (!releaseId)
            return reply.code(400).send(apiError(400, "缺少 releaseId"));
        try {
    ...
    app.post("/api/v1/admin/promote_script", { preHandler: authRequiredFn }, async (request, reply) => {
        const body = (request.body || {});
        const releaseId = String(body.releaseId || "");
        if (!releaseId)
            return reply.code(400).send(apiError(400, "旧版按脚本名发布已停用，请提供 releaseId"));
    ...
    app.post("/api/v1/admin/scripts/releases/:id/rollback", { preHandler: authRequiredFn }, async (request, reply) => {
        const releaseId = request.params.id;
        try {
    ...
    app.post("/api/v1/admin/rollback_script", { preHandler: authRequiredFn }, async (request, reply) => {
        const body = (request.body || {});
        const releaseId = String(body.releaseId || "");
        if (!releaseId)
            return reply.code(400).send(apiError(400, "旧版按脚本名回滚已停用，请提供 releaseId"));
        try {
            const result = await rollbackScriptReleaseFn(releaseId, request.adminUser || "admin");
            await recordRolledBackIssue(result, request.adminUser || "admin");
            return apiOk({ ...result, rolledBack: true });
    ...
    app.post("/api/v1/admin/scripts/releases/:id/disable", { preHandler: authRequiredFn }, async (request, reply) => {
        const releaseId = request.params.id;
        const body = (request.body || {});
        try {
    ...
    app.get("/api/v1/admin/script_history", { preHandler: authRequiredFn }, async (request) => {
        ...
        const releases = await query(`SELECT
         r.release_id,
         r.name,
         r.version,
         r.release_stage,
       ...
       LIMIT $2`, [scriptName, Math.max(limit * 2, 20)]);
 ...
 */
async function loadRuntimeLogsPayload(source, limit) {
    const files = {
    ...
    if (!backendResult.exists)
        missingSources.push("backend");
    if (!accessResult.exists)
        missingSources.push("nginx_access");
    if (!errorResult.exists)
        missingSources.push("nginx_error");
    const lines = [
...
/** 同步自动修复问题的发布阶段。 */
async function recordPublishedIssue(result, actor) {
    if (!result.issueId)
        return;
...
/** 同步自动修复问题的回滚阶段。 */
async function recordRolledBackIssue(result, actor) {
    if (!result.issueId)
        return;
...
function sendScriptReleaseError(reply, error) {
