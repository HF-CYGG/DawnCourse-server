...
export async function registerAdminRoutes(app, deps = {}) {
    ...
    app.post("/api/v1/admin/scripts/releases", { preHandler: authRequiredFn }, async (request) => {
        const body = (request.body || {});
    ...
    app.post("/api/v1/admin/promote_script", { preHandler: authRequiredFn }, async (request) => {
        const body = (request.body || {});
        const releaseId = String(body.releaseId || "");
        if (releaseId)
    ...
    app.post("/api/v1/admin/scripts/releases/:id/rollback", { preHandler: authRequiredFn }, async (request) => {
        const releaseId = request.params.id;
        await rollbackRelease(releaseId, request.adminUser || "admin");
        return apiOk({ rolledBack: true });
    });
    app.post("/api/v1/admin/rollback_script", { preHandler: authRequiredFn }, async (request) => {
        const body = (request.body || {});
...
async function publishRelease(releaseId, stage, actor) {
    ...
    await withTx(async (client) => {
        if (normalizedStage === "active") {
         ...
         WHERE script_id = $1 AND release_stage = 'active' AND release_id <> $2`, [row.script_id, releaseId]);
        }
        await client.query(`UPDATE script_releases
       SET release_stage = $2,
           ...
           approved_at = now(),
           published_at = now()
       WHERE release_id = $1`, [releaseId, normalizedStage, actor]);
...
async function rollbackRelease(releaseId, actor) {
    ...
    await withTx(async (client) => {
        ...
        if (row.parent_release_id) {
            await client.query(`UPDATE script_releases
         SET release_stage = 'rolled_back', channel = 'rolled_back', rollout_percent = 0
         WHERE script_id = $1 AND release_stage = 'active' AND release_id <> $2`, [row.script_id, row.parent_release_id]);
