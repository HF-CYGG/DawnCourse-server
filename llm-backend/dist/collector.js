...
export async function registerCollectorRoutes(app) {
    ...
    app.get("/api/v1/task_status", async (request) => {
        ...
        if (!task)
            return { status: "not_found", error: "task_not_found" };
        return {
            status: task.status,
...
export async function ingestParseReport(body, source) {
    ...
    if (queued) {
        ...
        const count = await query("SELECT sample_count FROM repair_issues WHERE issue_id = $1", [issueId]);
