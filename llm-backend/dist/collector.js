 ...
 */
export async function triggerRepairIfReady(input, deps = {}) {
    if (!input.queued || !input.hasSample)
        return false;
    ...
    const count = await queryFn("SELECT sample_count FROM repair_issues WHERE issue_id = $1", [input.issueId]);
    if (Number(count.rows[0]?.sample_count || 0) >= runtime.minQueueSize) {
        void startRepairJobFn(input.issueId, { actor: "collector", bypassMinQueue: false }).catch((error) => logErrorFn("repair job failed", { issueId: input.issueId, error: String(error) }));
        return true;
    }
 ...
 * - 两者都没有时，由上层创建新 issue。
 */
export function resolveIssueReuse(input) {
...
function normalizeSchoolProfileName(value) {
    return value.trim().replace(/\s+/g, "").toLowerCase();
}
async function upsertSchoolProfile(profile) {
    await query(`INSERT INTO school_profiles(school_id, school_name, normalized_name, school_system_type, source_hosts_json, status, created_from_issue_id)
...
function createMapCloudTaskStore(store) {
    return {
        async create(task) {
...
const pgCloudParseTaskStore = {
    ...
    async update(taskId, patch) {
        await query(`UPDATE cloud_parse_tasks
    ...
    async get(taskId) {
       ...
       FROM cloud_parse_tasks WHERE task_id = $1`, [taskId]);
 ...
 */
async function runCloudParseTask(taskId, content, summaryConfig, targetTaskStore = taskStore, targetCloudTaskStore = createMapCloudTaskStore(targetTaskStore)) {
 ...
 */
async function callCloudParseProvider(content, summaryConfig) {
 ...
 */
async function callCloudParseProviderOnce(content, strategy, summaryConfig) {
 ...
 */
function buildCloudParsePrompt(content, strategy) {
    const baseSystem = "你是课程表解析助手，只输出严格 JSON 数组，不要包含任何解释、Markdown、代码块。" +
    ...
    if (strategy === "strict") {
        return {
            ...
            userPrompt: "请严格提取并纠正字段类型，仅输出 JSON 数组。" +
    ...
    if (strategy === "repair") {
        return {
            ...
            userPrompt: "请执行“修复后提取”：合并同一课程重复行、修正周次/节次格式、补全可推断字段，最后仅输出 JSON 数组。\n" +
                `课表内容如下：\n${content}\n只输出 JSON 数组。`
        };
    }
    return {
        systemPrompt: baseSystem + "所有字段必须齐全，整数必须是数字类型。",
        userPrompt: "请从以下内容中提取课程信息，输出 JSON 数组，示例格式：" +
 ...
 */
async function callOpenAiCompatible(summaryConfig, systemPrompt, userPrompt) {
 ...
 */
function tryNormalizeJsonArray(rawText) {
    if (!rawText.trim())
        return null;
    try {
