...
export async function getRuntimeModelConfig(target) {
    const stored = await query("SELECT value_json FROM system_config WHERE key = 'model_config'");
...
export async function getAdminConfigPayload() {
    const stored = await query("SELECT value_json FROM system_config WHERE key = 'model_config'");
    const value = stored.rows[0]?.value_json || {};
...
export async function getRuntimePlatformConfig() {
    const stored = await query("SELECT value_json FROM system_config WHERE key = 'model_config'");
    const value = stored.rows[0]?.value_json || {};
    return {
...
export async function getManifestPublicBaseUrl(host) {
