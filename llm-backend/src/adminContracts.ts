 ...
 */
export const RUNTIME_LOG_SOURCE_KEYS = ["backend", "nginx_access", "nginx_error", "admin"] as const;

/**
 * 运行日志来源键类型。
 */
export type RuntimeLogSourceKey = (typeof RUNTIME_LOG_SOURCE_KEYS)[number];

/**
 * 后台缓存日志条目。
 * 这里与 `admin.ts` 内部缓冲区结构保持一致，便于转成可读文本行。
 */
export interface AdminBufferEntry {
  id: string;
  level: string;
  message: string;
...
/**
 * 运行日志来源明细。
 * 前端会把它渲染成“来源明细”说明，帮助运维理解每个来源是否可读。
 */
export interface RuntimeLogSourceDetail {
  key: RuntimeLogSourceKey;
  label: string;
  path: string;
  lineCount: number;
  missing: boolean;
...
export interface RuntimeLogPayload {
  ...
  loadedAt: number;
  lineCount: number;
  availableSources: RuntimeLogSourceKey[];
  sourceDetails: RuntimeLogSourceDetail[];
  files: Record<RuntimeLogSourceKey, string>;
  lines: string[];
 ...
 */
export const RUNTIME_LOG_SOURCE_LABELS: Record<RuntimeLogSourceKey, string> = {
  backend: "llm-backend",
 ...
 */
export function formatAdminBufferLine(entry: AdminBufferEntry): string {
...
/**
 * 历史事件基础输入。
 * 这里既兼容脚本发布记录，也兼容审计日志事件。
 */
export interface ScriptHistoryEventInput {
  type: string;
  createdAt: number;
  appliedBy?: string;
  schoolId?: string;
...
/**
 * 历史事件输出结构。
 * 字段命名与 `admin.js` 的 `normalizeScriptHistoryEntry()` 保持一致。
 */
 ...
 */
export function normalizeScriptHistoryEvent(input: ScriptHistoryEventInput): ScriptHistoryEntry {
...
export function buildScriptHistoryEntries(input: {
  ...
  const releaseIdsWithAudit = new Set(
    auditRows
  ...
  const releaseEvents = releaseRows
    ...
    .filter((item): item is ScriptHistoryEntry => Boolean(item));
 ...
 */
function normalizeIssueIds(issueId?: unknown, rawIssueIds?: unknown): string[] {
