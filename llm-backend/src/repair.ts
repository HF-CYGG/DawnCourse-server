...
interface IssueRow {
  ...
  affected_script_name: string;
  affected_category: string;
  affected_version: number;
  sample_count: number;
  failure_type: string;
}
...
async function runRepairJob(issueId: string, jobId: string, options: { actor?: string; bypassMinQueue?: boolean }): Promise<void> {
...
async function diagnose(issue: IssueRow, sample: string, baselineError: string): Promise<{ summary: string }> {
  return {
...
async function generateCandidate(
  ...
  sample: string,
  diagnosis: string
): Promise<{ script: string; summary: string; error?: string }> {
  ...
  if (!response.ok) return { script: "", summary: response.error || "model failed", error: response.error || "model_failed" };
...
async function callOpenAiCompatible(
  ...
  try {
    ...
    if (!response.ok) return { ok: false, text: "", error: json?.error?.message || `http_${response.status}` };
...
export function chatCompletionsUrl(provider: string, baseUrl: string): string {
...
function parseCandidate(text: string): { script: string; summary: string } {
...
async function loadIssue(issueId: string): Promise<IssueRow | null> {
  const result = await query<IssueRow>("SELECT * FROM repair_issues WHERE issue_id = $1", [issueId]);
...
async function saveRunnerReport(input: {
  ...
  sampleId?: string;
  targetType: string;
  report: RunnerResponse;
}): Promise<string> {
  const reportId = id("rr");
  await query(
    `INSERT INTO runner_reports(report_id, issue_id, job_id, script_id, release_id, sample_id, target_type, success, schema_valid,
