...
/** 脚本发布支持的作用域。 */
export type ScriptScopeKind = "global" | "system" | "school";

/** 唯一标识一条脚本发布轨道所需的字段。 */
export interface ScriptReleaseScope {
  targetType: string;
  category: string;
  name: string;
...
export function buildScriptKey(scope: ScriptReleaseScope): string {
  return [scope.targetType, scope.category, scope.name, scope.scopeKind, scope.scopeId].join("/");
}

/** 判断两个 release 是否属于同一条发布轨道。 */
export function sameScriptTrack(left: ScriptReleaseScope, right: ScriptReleaseScope): boolean {
  return buildScriptKey(left) === buildScriptKey(right);
}

/** 判断请求上下文能否使用指定作用域，空学校绝不匹配学校脚本。 */
export function releaseScopeMatchesRequest(scope: ScriptReleaseScope, systemType: string, schoolId: string): boolean {
  const normalizedSystem = normalizeSystemType(systemType);
  if (scope.scopeKind === "school") {
    if (!schoolId.trim() || schoolId.trim() !== scope.scopeId) return false;
