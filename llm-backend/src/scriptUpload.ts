...
import { buildScriptKey, type ScriptScopeKind } from "./scriptRelease.js";
import type { RunnerRequest, RunnerResponse, TargetType } from "./types.js";
...
type TxClient = { query: QueryLike };
type WithTxLike = <T>(callback: (client: TxClient) => Promise<T>) => Promise<T>;

export interface ScriptUploadInput {
  name: string;
  category: string;
  targetType: string;
  scopeKind: ScriptScopeKind;
  scopeId: string;
  ...
  testSchoolId?: string;
  pageStage?: string;
  manualValidationResult?: string;
  manualValidationPassed?: boolean;
}
...
interface ScriptUploadDependencies {
  query?: QueryLike;
  withTx?: WithTxLike;
  runScript?: (request: RunnerRequest) => Promise<RunnerResponse>;
  getActiveScriptContent?: typeof getActiveScriptContent;
}

export function validateScriptUploadInput(input: ScriptUploadInput): { ok: boolean; error: string } {
  ...
  if (category === "runtime") return { ok: false, error: "runtime_upload_forbidden" };
  if (!ALLOWED_CATEGORIES.has(category)) return { ok: false, error: "invalid_category" };
  if (!safeSegment(name) || !name.toLowerCase().endsWith(".js")) return { ok: false, error: "invalid_script_name" };
  if (!content.trim()) return { ok: false, error: "script_content_required" };
  if (Buffer.byteLength(content, "utf8") > MAX_UPLOAD_BYTES) return { ok: false, error: "script_too_large" };
  if (content.includes("\u0000") || content.includes("\uFFFD")) return { ok: false, error: "invalid_utf8_script" };
  if (scopeKind !== "school" && scopeKind !== "system") return { ok: false, error: "invalid_scope_kind" };
  if (scopeKind === "school" && !scopeId) return { ok: false, error: "school_scope_id_required" };
  if (scopeKind === "system" && normalizeSystemType(scopeId) === "UNKNOWN") {
    return { ok: false, error: "system_scope_id_required" };
  }
  if (category === "parsers" && targetType !== "parser") return { ok: false, error: "invalid_parser_target" };
  if (category === "js" && !ALLOWED_WEB_TARGETS.has(targetType)) return { ok: false, error: "invalid_web_target" };
  if (category === "js") {
    if (
      !String(input.testSchoolId || "").trim() ||
...
export async function createUploadedScriptRelease(
  rawInput: ScriptUploadInput,
  actor: string,
  deps: ScriptUploadDependencies = {}
  ...
  const withTxFn = (deps.withTx || withTx) as WithTxLike;
  ...
  const parentResult = input.parentReleaseId
    ? await queryFn<any>(
        "SELECT release_id FROM script_releases WHERE release_id = $1 AND script_key = $2 LIMIT 1",
        [input.parentReleaseId, scriptKey]
      )
    : await queryFn<any>(
        `SELECT release_id FROM script_releases
         WHERE script_key = $1 AND release_stage = 'active' AND status = 'enabled' AND kill_switch = false
         ORDER BY version DESC LIMIT 1`,
        [scriptKey]
      );
  if (input.parentReleaseId && !parentResult.rows[0]) throw new Error("parent_release_scope_mismatch");
  const parentReleaseId = String(parentResult.rows[0]?.release_id || "");

  const versionResult = await queryFn<any>(
    "SELECT COALESCE(MAX(version),0) AS version FROM script_artifacts WHERE script_key = $1",
    [scriptKey]
  );
  ...
  await withTxFn(async (client) => {
    ...
    if (dependency?.releaseId) {
      await client.query(
        "INSERT INTO script_release_dependencies(release_id,dependency_release_id,load_order) VALUES ($1,$2,0) ON CONFLICT DO NOTHING",
        [releaseId, dependency.releaseId]
      );
    }
    if (input.category === "js") {
      await client.query(
        `INSERT INTO script_manual_validations(validation_id,release_id,school_id,page_stage,result_note,passed,actor)
         VALUES ($1,$2,$3,$4,$5,true,$6)`,
        [id("val"), releaseId, input.testSchoolId, input.pageStage, input.manualValidationResult, actor]
      );
    }
    await client.query(
      "INSERT INTO audit_logs(actor,action,target_type,target_id,detail_json) VALUES ($1,'upload_release','script_release',$2,$3::jsonb)",
      [actor, releaseId, JSON.stringify({ scriptKey, version, validationReport })]
    );
  });
  return { releaseId, scriptKey, version, validationStatus: "passed", validationReport };
}

export async function revalidateScriptRelease(
  releaseId: string,
  manualInput: Partial<ScriptUploadInput>,
  actor: string,
  deps: ScriptUploadDependencies = {}
): Promise<ScriptUploadResult> {
  const queryFn = (deps.query || query) as QueryLike;
  ...
  const releaseResult = await queryFn<any>(
    `SELECT r.*,a.content FROM script_releases r
  ...
  const input = normalizeInput({
    ...
    scopeKind: String(release.scope_kind || "") as ScriptScopeKind,
  ...
  try {
    ...
    await queryFn(
      "UPDATE script_releases SET validation_status = 'passed',validation_report_json = $2::jsonb WHERE release_id = $1",
      [releaseId, JSON.stringify(validationReport)]
    );
    if (input.category === "js") {
      await queryFn(
        `INSERT INTO script_manual_validations(validation_id,release_id,school_id,page_stage,result_note,passed,actor)
    ...
    await queryFn(
      "INSERT INTO audit_logs(actor,action,target_type,target_id,detail_json) VALUES ($1,'revalidate_release','script_release',$2,$3::jsonb)",
      [actor, releaseId, JSON.stringify({ passed: true, validationReport })]
    );
    return {
      releaseId,
    ...
    await queryFn(
      "UPDATE script_releases SET validation_status = 'failed',validation_report_json = $2::jsonb WHERE release_id = $1",
      [releaseId, JSON.stringify(report)]
    );
    await queryFn(
      "INSERT INTO audit_logs(actor,action,target_type,target_id,detail_json) VALUES ($1,'revalidate_release','script_release',$2,$3::jsonb)",
      [actor, releaseId, JSON.stringify(report)]
    );
    throw error;
  }
}

function normalizeInput(input: ScriptUploadInput): ScriptUploadInput & {
  scopeKind: "system" | "school";
  schoolSystemType: string;
} {
  const scopeKind = input.scopeKind as "system" | "school";
  const normalizedSystem = normalizeSystemType(
    scopeKind === "system" ? input.scopeId : input.schoolSystemType
  );
  return {
    ...input,
...
async function validateParserCandidate(
  input: ReturnType<typeof normalizeInput>,
  queryFn: QueryLike,
  runScriptFn: (request: RunnerRequest) => Promise<RunnerResponse>,
  getActiveFn: typeof getActiveScriptContent
): Promise<Record<string, unknown>> {
  const sampleSchoolId = input.scopeKind === "school" ? input.scopeId : "";
  const samples = await queryFn<any>(
    `SELECT sample_id,sanitized_content,school_id,school_system_type
    ...
    [sampleSchoolId, input.schoolSystemType]
  );
  if (!samples.rows.length) throw new Error("validation_sample_required");
  const harness = await getActiveFn("runtime", "script_host.js");
  if (!harness?.content) throw new Error("harness_missing");
  ...
  for (const sample of samples.rows) {
    const report = await runScriptFn({
      ...
      targetType: "parser" as TargetType,
      timeoutMs: 5_000
    });
    reports.push({
